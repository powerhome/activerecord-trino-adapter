...
RSpec.describe ActiveRecord::ConnectionAdapters::Trino::DatabaseStatements do
  ...
  describe "#exec_query" do
    ...
    it "refuses non-empty bind variables" do
      ...
      end.to raise_error(ActiveRecord::Trino::Error, /bind variables are not supported/)
    end
  ...
  describe "slow-query notification" do
    it "emits active_record_trino.slow_query when threshold is exceeded" do
      stub_trino_query(sql: "SELECT 1", columns: [%w[n integer]], rows: [[1]])

      adapter.instance_variable_set(:@slow_query_threshold, 0.0)

      payload = nil
      ActiveSupport::Notifications.subscribed(->(*args) { payload = args.last }, "active_record_trino.slow_query") do
    ...
    it "does not emit when threshold is not exceeded" do
      ...
      emitted = false
      ActiveSupport::Notifications.subscribed(->(*_args) { emitted = true }, "active_record_trino.slow_query") do
        adapter.execute("SELECT 1")
      end
    ...
    it "carries sql and a numeric duration in the notification payload" do
      ...
      payloads = []
      subscriber = ActiveSupport::Notifications.subscribe("active_record_trino.slow_query") do |*args|
        payloads << args.last
      end
