...
RSpec.describe ActiveRecord::ConnectionAdapters::Trino::TypeMap do
  ...
  describe "date / time / timestamp" do
    ...
    it "maps timestamp with time zone to ActiveRecord::Trino::Type::TimestampWithZone" do
      expect(type_map.lookup("timestamp with time zone"))
        .to be_a(ActiveRecord::Trino::Type::TimestampWithZone)
  ...
  describe "json" do
    it "maps json to ActiveRecord::Trino::Type::Json" do
      expect(type_map.lookup("json")).to be_a(ActiveRecord::Trino::Type::Json)
    end
  end
  ...
  describe "composite types" do
    ["array(varchar)", "map(varchar, integer)", "row(a varchar, b integer)"].each do |t|
      it "returns Unsupported for #{t}" do
        ...
        expect(type).to be_a(ActiveRecord::Trino::Type::Unsupported)
        expect { type.cast("anything") }.to raise_error(ActiveRecord::Trino::UnsupportedTypeError)
      end
    end
  end
  ...
  describe "unknown type" do
    it "returns Unsupported and raises on cast" do
      ...
      expect(type).to be_a(ActiveRecord::Trino::Type::Unsupported)
      expect { type.cast("v") }.to raise_error(ActiveRecord::Trino::UnsupportedTypeError, /hyperdrive/)
    end
  end
