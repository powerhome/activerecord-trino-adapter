  ...
  ActiveRecord::ConnectionAdapters::Trino::ReadOnly::WRITE_METHODS.each do |method|
    it "raises ReadOnlyError for ##{method}" do
      expect { host.public_send(method) }
        .to raise_error(ActiveRecord::Trino::ReadOnlyError, /#{method}/)
    end
  end

  it "includes a hint that the gem is read-only in the error message" do
    expect { host.insert }
      .to raise_error(ActiveRecord::Trino::ReadOnlyError, /read-only/)
  end
end
